		...
		<div class="x_content" style="padding: 36px 25%">
			<div class="row">
				<div class="col-md-12 col-sm-12 col-xs-12">
					<div class="x_title">
						<h2>
							ลบสินค้า #{{$pd->code}} {{$pd->name}} {{$pd->color}}
						</h2>
					...
					<form style="margin-top: 16px" action="{{ route('products.destroy',$pd->id) }}" method="POST" onsubmit="return confirm('หากคุณลบรายการแล้ว จะไม่สามารถกู้คืนได้ ยืนยันทำต่อ?')">
						{{ csrf_field() }}
						{{ method_field('delete') }}
						<div class="form-group">
							<label for="email">Enter : <code>delete this item #{{$pd->id}}</code></label>
...
<script type="text/javascript">
	$(document).ready(()=>{
		$('#confirm-input').keyup(function(){
			if($(this).val() === 'delete this item #{{$pd->id}}'){
